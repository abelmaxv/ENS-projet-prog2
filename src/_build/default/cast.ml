...
and expr =
  ...
  | OP1 of mon_op * loc_expr (* OP1(mop, e) dénote -e, ~e, e++, e--, ++e, --e, &e ou *e. *)
...
type var_declaration =
    CDECL of location * string * ctyp (* declaration de variable de type ctyp. *)
...
let getloc () = 
  let begloc = {pos_fname = !cfile; pos_lnum = !oldcline; pos_bol = !oldccol; pos_cnum = !oldccol} in
  let endloc = {pos_fname = !cfile; pos_lnum = !cline; pos_bol = !ccol; pos_cnum = !ccol} in
  begloc, endloc
...
let rec bufout_expr buf pri e =
	...
	VAR s -> Buffer.add_string buf s
      ...
      | SET_VAL (x, e) -> (bufout_open buf pri setop_prec;
				 Buffer.add_string buf "*";
				 Buffer.add_string buf x;
				 Buffer.add_string buf "=";
				 bufout_loc_expr buf setop_prec e;
